...
#Admin buttons -- currently only support Exit program function
#later should add password check or secret barcode scan to get in
# Other functions modify files -- add new students and change permission
#Admin functions -- modify files and exit program
def Admin(root):
...
def main():
    ...
    my_pic = Image.open('GA.png')
    render = my_pic.resize((1600, 620), Image.ANTIALIAS)
    new_pic = ImageTk.PhotoImage(render)
    label = Label(frame, image = new_pic)
    label.place(relwidth =1, relheight=1)


    frame0 = Frame(root, bd=5)
    frame0.place(relx=0.5, rely=0.68, relwidth=1, relheight=0.1, anchor='s')
    label0 = Label(frame0, text='GA Sign-In and Sign-Out System Faculty Beta')
    label0.config(font=('Helvetica 48 bold'))
    label0.place(relwidth =1, relheight=1)

    frame1 = Frame(root, bd=5)
    frame1.place(relx=0.4, rely=0.75, relwidth=0.75, relheight=0.05, anchor='s')
    label1 = Label(frame1, text='Please click on a button to sign-in or sign-out.')
    label1.config(font=('Helvetica 25 italic'), fg ='grey')
    label1.place(relwidth =1, relheight=1)


    frame2 = Frame(root, bd=5)
    frame2.place(relx=0.5, rely=0.95, relwidth=0.88, relheight=0.04, anchor='s')
    credit1 = Label(frame2, text=' An Advanced Topics in CS Project created by Sam Wang under the guidance of Mr. Oswald, Ms. Kennedy, and Mr. DiFranco.')
    credit1.config(font=('Arial 18'), fg ='blue')
    credit1.place(relwidth =1, relheight=1)




    #Create buttons
    frame3 = Frame(root, bg='#80c1ff', bd=3)
    frame3.place(relx=0.2, rely=0.86, relwidth=0.25, relheight=0.09, anchor='s')
    btn1 = Button(frame3, text ="Lateness", font ='Helvetica 36 bold', command= SignIn)
    btn1.place(relwidth =1, relheight=1)



    frame4 = Frame(root, bg='#80c1ff', bd=3)
    frame4.place(relx=0.52, rely=0.86, relwidth=0.25, relheight=0.09, anchor='s')
    btn2 = Button(frame4, text ="Off Campus", font ='Helvetica 36 bold', command= SignOut)
    btn2.place(relwidth =1, relheight=1)


    frame5 = Frame(root, bg='grey', bd=3)
    frame5.place(relx=0.85, rely=0.86, relwidth=0.15, relheight=0.09, anchor='s')
    btn3 = Button(frame5, text ="Admin",font ='Helvetica 36 bold', command=lambda:Admin(root))
    btn3.place(relwidth =1, relheight=1)
